...
std::vector<std::vector<cv::cuda::GpuMat>> yolov8_gpu::GpuInference::preprocess(const cv::cuda::GpuMat &gpuImg) {
...
std::vector<yolov8_gpu::Object> yolov8_gpu::GpuInference::detectObjects(const cv::cuda::GpuMat &inputImageBGR) {
...
std::vector<yolov8_gpu::Object> yolov8_gpu::GpuInference::detectObjects(const cv::Mat &inputImageBGR) {
...
std::vector<yolov8_gpu::Object> yolov8_gpu::GpuInference::postProcessSegmentation(std::vector<std::vector<float>> &featureVectors) {
    const auto &outputDims = m_trtEngine->getOutputDims();
...
std::vector<yolov8_gpu::Object> yolov8_gpu::GpuInference::postprocessPose(std::vector<float> &featureVector) {
    const auto &outputDims = m_trtEngine->getOutputDims();
...
std::vector<yolov8_gpu::Object> yolov8_gpu::GpuInference::postprocessDetect(std::vector<float> &featureVector) {
    const auto &outputDims = m_trtEngine->getOutputDims();
    auto numChannels = outputDims[0].d[1];
    auto numAnchors = outputDims[0].d[2];
...
void yolov8_gpu::GpuInference::drawObjectLabels(cv::Mat &image, const std::vector<Object> &objects, unsigned int scale) {
